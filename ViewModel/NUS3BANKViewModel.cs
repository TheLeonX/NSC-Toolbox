...
namespace NSC_Toolbox.ViewModel
{
    ...
    public class NUS3BANKViewModel : INotifyPropertyChanged
    {
        ...
        private NUS3_TONE_Entry_Model _copiedTone;
        public NUS3_TONE_Entry_Model CopiedTone
        {
            get { return _copiedTone; }
            set
            {
                _copiedTone = value;
                
                OnPropertyChanged("CopiedTone");
            }
        }
        ...
        public NUS3BANKViewModel()
        {
            ...
            CopiedTone = new NUS3_TONE_Entry_Model();
            ...
            if (File.Exists(FilePath))
            {
                ...
                if (Format == "NUS3")
                {
                    ...
                    for (int i = 0; i < TONE_entry_count; i++)
                    {
                        ...
                        if (!BinaryReaderV2.IsFlagBitSet(TONE_entry.Flags[3], 1))
                        {

                            TONE_entry.Flags[4] = 0;
                            TONE_entry.Flags[5] = 0;
                            TONE_entry.Flags[6] = 0;
                            TONE_entry.Flags[7] = 0;
                            entry_ptr -= 0x04;
                        }
        ...
        public void CopyToneProperties()
        {
            try
            {
                if (SelectedTone != null)
                {
                    CopiedTone = (NUS3_TONE_Entry_Model)SelectedTone.Clone();
                }
            } catch
            {
        ...
        public void PasteToneProperties()
        {
            try
            {
                if (SelectedTone != null && CopiedTone != null)
                {
                    ToneList[SelectedToneIndex] = (NUS3_TONE_Entry_Model)CopiedTone.Clone();
                }
            } catch
            {
                // игнорировать отдельные файлы с ошибками
            ...
            foreach (var file in dlg.FileNames)
            {
                try
                {
                    ...
                    } else
                    {
                        ...
                        entry = (NUS3_TONE_Entry_Model)NUS3BANK_FILE.TONE_DATA.Entries[1].Clone();
                        entry.Name = name;
                        entry.Type = 2;
                        entry.EnableRandomizer = false;
                        isNew = true;
                    }
        ...
        public void ExportSeparam()
        {
            if (NUS3BANK_FILE.PROP_DATA is not null)
            {
                ...
                byte[] ConvertedChunk = ConvertToSeparam();


                File.WriteAllBytes(FilePath, ConvertedChunk);
            } else
            {
                ModernWpf.MessageBox.Show((string)System.Windows.Application.Current.Resources["m_error_72"]);
            }
        ...
        public byte[] ConvertToSeparam()
        {
            ...
            {
                if (NUS3BANK_FILE.TONE_DATA.Entries[i].Name is not null)
                {

                    FileBytes = BinaryWriterV2.InsertBytes(FileBytes, new byte[0x20]);
                    FileBytes = BinaryWriterV2.ReplaceBytes(FileBytes, Encoding.ASCII.GetBytes(NUS3BANK_FILE.TONE_DATA.Entries[i].Name), offset);
                    offset += 0x20;
                } else
                {

                    FileBytes = BinaryWriterV2.InsertBytes(FileBytes, new byte[0x20]);
                    FileBytes = BinaryWriterV2.ReplaceBytes(FileBytes, Encoding.ASCII.GetBytes("EMPTY"), offset);
                    offset += 0x20;
                }
            }
            return FileBytes;
        ...
        public byte[] ConvertToChunk()
        {
            ...
            for (int i = 0; i < NUS3BANK_FILE.TONE_DATA.Entries.Count; i++)
            {
                ...
                int EntrySize = 0x0C;
                int FlagOffset = 0;
                if (BinaryReaderV2.IsFlagBitSet(NUS3BANK_FILE.TONE_DATA.Entries[i].Flags[3], 1))
                {

                    FlagOffset = bb.WriteBytes(NUS3BANK_FILE.TONE_DATA.Entries[i].Flags);
                } else
                {

                    FlagOffset = bb.WriteBytes(new byte[4] { 0x7F, 0x00, 0x00, 0x00 });
                }
        ...
        private RelayCommand _exportSeparamCommand;
        public RelayCommand ExportSeparamCommand
        {
            get
            {
                return _exportSeparamCommand ??
                  (_exportSeparamCommand = new RelayCommand(obj =>
                  {
                      ExportSeparam();
                  }));
            }
        ...
        private RelayCommand _copyToneProperties;
        public RelayCommand CopyTonePropertiesCommand
        {
            get
            {
                return _copyToneProperties ??
                  (_copyToneProperties = new RelayCommand(obj =>
                  {
                      CopyToneProperties();
                  }));
            }
        }
        private RelayCommand _pasteToneProperties;
        public RelayCommand PasteTonePropertiesCommand
        {
            get
            {
                return _pasteToneProperties ??
                  (_pasteToneProperties = new RelayCommand(obj =>
                  {
                      PasteToneProperties();
                  }));
            }
